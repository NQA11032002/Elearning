                ...
                <form action="" @submit.prevent="insertTopicCourse">
                    <div class="flex justify-between max-sm:flex-col shadow-md">
                        <div class="w-full flex flex-col justify-center max-sm:w-full max-sm:p-2" style="margin-left: 50%; transform: translateX(-50%);">
                            ...
                            <input v-model="course.id" disabled class="outline-none py-2 border px-2 focus:border-blue-900 w-full rounded max-sm:w-full" type="text" placeholder="Mã khóa học:">
            
                            <span class="py-2 font-semibold">Tên chuyên đề:</span>
                            <input v-model="course.name" class="outline-none border py-2 px-2 focus:border-blue-900 w-full rounded max-sm:w-full" type="text" placeholder="Tên chuyên đề:">
            
                            <div class="flex justify-end w-full my-4 max-sm:w-full">
                                <a :href="'/detail-course/' + course.id" class="bg-gray-600 h-10 text-white p-2 mr-4 rounded text-sm max-sm:mr-2 w-1/5 text-center"><button >Hủy</button></a>
                                <button type="submit" class="bg-blue-900 text-white p-2 h-10 w-2/5 rounded text-sm max-sm:mr-0">Lưu chuyên đề</button>
                            </div>
                        </div>
                    </div>
                </form>
    ...
    methods: {
        ...
        async insertTopicCourse(){
            ...
            if(res.data.data != null){
                ...
                this.contents.step = "/expert/course-lesson/" + res.data.data.id;
            }
        }
    },
    data() {
